...
class TransactionRoomReservationController extends Controller
{
    ...
    public function payDownPayment(Customer $customer, Room $room, Request $request, TransactionRepository $transactionRepository, PaymentRepository $paymentRepository)
    {
        ...
        //Printing::printer('default')->send($pdfFilePath, 'Invoice.pdf');

        $html = view('pdf.invoice', compact('payment'))->render();
        ...
        $pdfFile = 'receipt' . $payment->id . '.pdf';

        // Save the generated PDF to storage
        Storage::put($pdfFile, $dompdf->output());

        // Send the PDF file to the printer
        //Printing::print(storage_path($pdfFile));
        //$printer = new Printer('default');
       // $printJob = Printing::newPrintTask()->printer($printer)->file(storage_path('app/' . $pdfFile))->send();

       // Printing::printer('default')->send(storage_path('app/' . $pdfFile), 'Invoice.pdf');

       // $pdfContent = $dompdf->output();

        try {
            // Use the default printer URI from your configuration
            $printerUri = config('printing.printers.default.uri');

            // Create a new print task and set the printer URI
            $printTask = Printing::newPrintTask();
            $printTask->printer($printerUri);

            // Set the content to print and send the print job
            $printTask->file(storage_path('app/' . $pdfFile));
            $printJob = $printTask->send();

            // Optionally, you can retrieve and log the print job ID
            $printJobId = $printJob->id();

           // return response()->json(['message' => 'PDF content sent to printer successfully']);
        } catch (\Exception $e) {
